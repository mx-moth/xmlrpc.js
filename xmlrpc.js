...
(function() {
	...
	var XMLRPCFault = XMLRPC.XMLRPCFault = function() {
	...
	*/
	...
	XMLRPC.makeType('nil', false,
		function(_, mkel) { return mkel('nil'); },
	...
	XMLRPC.makeType('struct', false, function(value, mkel) {
		...
		return A.reduce(descendants(node, 'member'), function(struct, el) {
			var key = descendant(el, 'name').textContent;
			var value = XMLRPC.parseValue(descendant(el, 'value'));
